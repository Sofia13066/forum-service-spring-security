...
public class UserAccountServiceImpl implements UserAccountService, CommandLineRunner {
	...
	@Override
	public void changePassword(String login, String newPassword, LocalDate dataExpiredPassword) {
		UserAccount userAccount = repository.findById(login).orElseThrow(() -> new UserNotFoundException());
		String password = passwordEncoder.encode(newPassword);
		userAccount.setPassword(password);
		userAccount.setDateExpiredPassword(dataExpiredPassword);
		repository.save(userAccount);
	}
	...
	public void run(String... args) throws Exception {
		if(!repository.existsById("admin")) {
			...
			UserAccount userAccount = new UserAccount("admin", password , null, "", "");
